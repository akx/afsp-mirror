...
Author / revision:
  ...
  $Revision: 1.102 $  $Date: 2020/11/25 17:54:49 $
...
#define MAGIC_S(ftype, lhead, offs, string) \
                {(ftype), (lhead), \
                {(offs), (string), (int) sizeof(string) - 1}, \
                {0L, NULL, 0}}
#define MAGIC_D(ftype, lhead, offs1, string1, offs2, string2) \
                {(ftype), (lhead), \
                {(offs1), (string1), (int) sizeof(string1) - 1}, \
                {(offs2), (string2), (int) sizeof(string2) - 1}}
...
/* ===== Unsupported audio file formats */
  ...
  /* FLAC file */
  MAGIC_S(FTU_CODE(23), 4, 0L, "fLaC"),
  /* OGG file */
  MAGIC_S(FTU_CODE(24), 4, 0L, "OggS")
};
