...
Routine:
  int AFCheckDataPar(FILE *fp, int Lw, long int *Dstart,
...
Description:
  ...
  file, less the size of the header. Ldata is checked against the actual size of
  the file. It is an error for this value to exceed the actual space in the
  file.

  The number of samples, the data element size, and the number of channels are
  used to calculate the actual amount of space space occupied by the data. If
  this value exceeds the space available, this is an error. If Nsamp is
...
Author / revision:
  ...
  $Revision: 1.9 $  $Date: 2020/11/29 23:17:27 $
