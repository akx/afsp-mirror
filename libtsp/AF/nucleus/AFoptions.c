...
Description:
  ...
  options structure is defined in the header file AFpar.h. Routines are provided
  to set and access some of the fields.
  ...
  Program data full scale (double ScaleV):
    ...
    output file, the inverse scaling is used. The default value for ScaleV is 1.
  ...
  Number of samples (long int NsampND):
    ...
    value is not available. The NsampND options are as follows.
      ...
      1 - AFopnRead returns the number of samples if the file is seekable or the
          number of is specified in the file header. Otherwise return a value
          indicating that this value is undefined (AF_NSAMP_UNDEF).
  Random access requirement (int RAccess):
    Some programs require the ability to reposition the file to an earlier
    position (to reread data, for instance). This parameter controls whether a
    file will be opened if it does not support random access (viz. file streams
    from a pipe are not random access).
  ...
  Number of frames (long int Nframe):
    ...
    not predefined. For some types of output files, this setting means that the
    output file must be random access.
  ...
  Speaker configuration (unsigned char *SpkrConfig):
    This null-terminated string contains the speaker location codes. This string
    can be set with symbolic values using the routine AFsetSpeaker. This string
    is allocated as needed. It can be deallocated by invoking AFsetSpeaker ("").
    The default is that no speaker locations are specified.
  ...
  Header information structure (struct AF_info Uinfo):
    ...
    routine AFsetInfo. Storage is allocated as needed.  It can be deallocated by
    invoking AFsetInfo (""). See AFsetInfo for more information.
...
Parameters:
  ...
  <-  struct AF_opt *AFoptions
   ...
   -> unsigned int Cat
      Categories of options to be reset. Cat is the sum of the following
      sub-categories:
...
Author / revision:
  ...
  $Revision: 1.36 $  $Date: 2020/11/26 11:33:35 $
