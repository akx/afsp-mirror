...
Routine:
  ...
  int AFfRdMulaw (AFILE *AFp, float Dbuff[], int Nreq)
  int AFfRdMulawR (AFILE *AFp, float Dbuff[], int Nreq)
...
Purpose:
  ...
  Read bit-reversed 8-bit mu-law data from an audio file (return float values)
...
Author / revision:
  ...
  $Revision: 1.4 $  $Date: 2017/09/07 16:41:32 $
...

{
  ...
  */
  ...
  for (is = 0; is < Nreq; ) {
    ...
    /* Scale */
    for (i = 0; i < Nr; ++i, ++is)
      Dbuff[is] = (float) (AFp->ScaleF * Atab[Buf[i]]);
...

{
  ...
  */
  ...
  for (is = 0; is < Nreq; ) {
    ...
    /* Convert and scale */
    for (i = 0; i < Nr; ++i, ++is)
      Dbuff[is] = (float) (AFp->ScaleF * Mutab[Buf[i]]);
...

{
  ...
  */
  ...
  for (is = 0; is < Nreq; ) {
    ...
    /* Convert and scale */
    for (i = 0; i < Nr; ++i, ++is)
      Dbuff[is] = (float) (AFp->ScaleF * MuRtab[Buf[i]]);
