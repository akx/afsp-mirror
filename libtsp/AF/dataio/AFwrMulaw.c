...
Routine:
  int AFwrMulaw (AFILE *AFp, const float Fbuff[], int Nval)
...
Author / revision:
  ...
  $Revision: 1.36 $  $Date: 2001/03/07 03:32:52 $

-------------------------------------------------------------------------*/

static char rcsid [] = "$Id: AFwrMulaw.c 1.36 2001/03/07 AFsp-v6r8 $";
...
/* Write data to the audio file */
  ...
  while (is < Nval) {
    ...
    for (i = 0; i < N; ++i) {
      Dv = AFp->ScaleF * Fbuff[i+is];
      if (Dv > AMAX || Dv < -AMAX)
	++AFp->Novld;
      Buf[i] = Yq[SPfQuantL (Dv, Xq, NLEV)];
    }
