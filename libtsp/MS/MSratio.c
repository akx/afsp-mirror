...
Description:
  A continued fraction expansion approximation to a given floating point value
  is calculated iteratively. At successive stages in the expansion, the
  approximation error for the convergents alternates in sign but with
  ...
  1: The magnitude of either the numerator or the denominator of the
     ...
     convergents are tested to see if one provides a smaller error than backing
     off to the convergent from the previous iteration.
  ...
  exceeding MaxN or the denominator exceeding MaxD. The resulting fraction N/D
  is the fraction nearest Val amongst those fractions with numerator and
...
Author / revision:
  ...
  $Revision: 1.12 $  $Date: 2020/11/12 17:33:13 $
...
/*
  The continued fraction expansion of a value x is of the form
    x = a[0] +  1
      ...
      = {a[0]; a[1], a[2], ... }
  The a[k] are integers (all positive, except possibly for a[0]). The value a[0]
  ...
  ratio of integers) or indefinitely (x is irrational).
  ...
  Properties of the convergent C[k] = p[k]/q[k]:
   1: The convergent C[k] has the property that among fractions with denominator
      less than or equal to q[k], the C[k] is the one nearest to x.
  ...
  The secondary convergents are either increasing (n odd) or decreasing (n
  even). The denominators are increasing in this sequence. No other fraction can
  be placed into this sequence and maintain these properties.

  The closest fraction to x which has a denominator less than a given value M is
  either a convergent or a secondary convergent.
*/
...
/* Main loop for the continued fraction expansion */
  while (1) {
    ...
    /* Next term
       (AV should be positive, since the denominator is (AV - floor(AV)). If it
       is negative, we are being limited by machine precision.)
    */
    AV = 1.0 / (AV - ak);
    if (AV < 0.0)
      break;
