...
Author / revision:
  ...
  $Revision: 1.21 $  $Date: 2001/10/11 13:52:19 $

-------------------------------------------------------------------------*/

static char rcsid[] = "$Id: CPcopyChan.c 1.21 2001/10/11 AFsp-v6r8 $";
...
/* *** offsets and counts are sample offsets, not frame values *** */
  ...
  while (Nrem > 0L) {
    ...
    Nr = AFreadData (AFpI, offr + Nj * StartF, Fbuf, Nv);
...
/* Split the buffer space up for the input and output buffers */
  ...
  while (Nrem > 0) {
    ...
    for (j = 0; j < Nifiles; ++j) {
      ...
      AFreadData (AFp[j], offs, Fbufi, Nfr * Nj);
      for (k = 0; k < Nj; ++k) {
	for (i = 0; i < Nfr; ++i)
	  Fbufo[i*NO+n+k] = Fbufi[i*Nj+k];
      }
      n += Nj;
    }
