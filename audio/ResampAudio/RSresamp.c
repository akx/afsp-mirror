...
Description:
  This routine resamples data from one audio file and writes it to another audio
  file.
...
Author / revision:
  ...
  $Revision: 1.23 $  $Date: 2020/11/25 01:43:57 $
...
/*
  ...
  a transversal filter h(t). Then we can form a composite continuous-time filter
  g(t) by convolving the linear interpolation filter with h(t). The impulse
  response of the resulting filter is that of h(n) with linear interpolation
  ...
  response occurs at 80 kHz. This tends to suppress the image of the response at
  80 kHz. This suppression is incomplete however since the image extends 4 kHz
  on each side of 80 kHz. The worst suppression is for a frequency 4 kHz below
  80 kHz. In addition, the desired frequencies surrounding zero are subject to a
  slight lowpass effect, this effect being worst at 4 kHz.
  ...
  suppression at the bottom of the first image. The suppression at other image
  frequencies will be better. The following table shows the results for an input
  sampling rate fs and various values of Ir.
  ...
  Note that if we use sample-and-hold instead of linear interpolation (i.e. use
  the value of the closest bracketing sample), the attenuation will be half of
  these dB values.
*/

/*
  For the k'th output sample, the beginning of the filter is aligned with the
  input array at time t(k) (measured in samples of the input data),
    t(k) = t(0) + k * dt,
  where dt is the spacing between output samples. The data is processed frame by
  frame. Consider frame j,
  ...
  Ds = 1 and set Ns to be a floating point value. If dt is rational, we adopt a
  time update procedure that prevents round-off error from accumulating.
  ...
  which is a multiple of 1/Ir. The filter is used twice, once to the left of the
  desired position and once to the right of the desired position.

  The input signal x(.) is the data in the file, with x(0) corresponding to the
  first data value in the file. Conceptually a new increased rate signal, xi(.),
  is formed from the input signal x(.) by inserting Ir-1 zeros between each
  element of x(.),
    xi(l*Ir) = x(l),
  ...
  {h[0],...,h[Ncof-1]} with the data elements {xi(m),...,xi(m-Ncof+1)}. Because
  of the zeros in xi(.), the dot product can use the vectors
  {h[mr],h[mr+Ir],...} and {x(l),x(l-1),...}.

  An output sample will be calculated at position Ir*t in the signal xi(.). This
  position will be bracketed by ml(t) and mu(t) where ml(t) and mu(t) are
  integer values.
...
/* Multi-channel data:
   ...
   multi-channel data, each channel is filtered separately with its own memory.
