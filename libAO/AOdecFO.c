...
Description:
  This routine decodes an option for an output audio file. The routine AOinitOpt
  must be called first to initialize the option arguments. If this routine fails
  to find an input audio file option, the argument pointer is reset to allow
  another routine to try to decode the option.
...
Author / revision:
  ...
  $Revision: 1.33 $  $Date: 2020/11/23 18:35:18 $
...
/*
  ...
  structure is set. These options will be picked up when an output file is
  opened.
    Speaker locations: The corresponding audio file option is set, viz.
      AFopt->SpkrConfig
    Data format, number of bits/sample: The corresponding audio file option is
    set, viz.
      AFopt->NbS
...
Description:
  ...
  corresponding data format code. In the case of an invalid specification, this
  routine stops with an error message. For use in the error message, the
  program name should be set using the routine UTsetProg.
